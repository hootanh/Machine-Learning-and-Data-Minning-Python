...
class MouseDataPlot(MouseData):
    ...
    def display(self, fig=None, **kwargs):
        ...
        if fig is None:
            ...
            self.ax = plt.gca()
            self.ax.axis(self.lim); self.ax.set_xticks([]); self.ax.set_yticks([]);
        else:
            self.fig = fig
            self.ax  = fig.gca()
            self.lim = ax.axes()
        ...
        def on_click(event):
            if (event.button == 1):     # left click: add data point
                ...
                ml.plotClassify2D(None,self.X,self.Y,ax=self.ax)
            elif (event.button == 3):   # right click: remove data point
                #self.lim = self.ax.axis();
                ...
                self.ax = self.fig.gca(); self.ax.axis(self.lim);        
                ml.plotClassify2D(None,self.X,self.Y,ax=self.ax)
            else: pass
    ...
    def update_plot(self):
        ...
        if (self.m > 0) or (self.plot is not None):
            if (self.plot is not None):
                ...
                plotClassify2D(None, self.X,self.Y) # add options?
